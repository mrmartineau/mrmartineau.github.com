...
function loadUserAndRepo(username, type, id) {
	...
	i++;
	$.ajax({
		url: "https://api.github.com/users/" + username +"/" + type +"?page="+ i +"&per_page=100&callback=?",
		context: id,
		dataType: "jsonp",
		success: function(data){
			...
			if (data.length = 0) {
				//$('<h1>No more repos</h1>').appendTo(id);
				return false;
			}
...
function loadGists(username, type, id) {
	...
	j++;
	$.ajax({
		url: "https://api.github.com/"+ type +"?page="+ j +"&per_page=100&callback=?",
		context: id,
		dataType: "jsonp",
		success: function(data){
			var items = [];
